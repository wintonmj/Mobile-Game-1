...
class AnimationService {
  ...
  constructor(registry: IRegistry) {
    ...
    this.subscriptions.push(
      this.eventBus.on<{x: number, y: number}>('player.moved', this.handleMovementAnimation.bind(this)),
      this.eventBus.on<{x: number, y: number}>('player.collision', this.handleCollisionAnimation.bind(this)),
      this.eventBus.on<{weaponId: string, animationKey: string}>('weapon.used', this.handleWeaponAnimation.bind(this)),
      this.eventBus.on<{entityId: string, damage: number}>('enemy.damaged', this.handleDamageAnimation.bind(this))
    );
  }
...
class UIController {
  ...
  constructor(registry: IRegistry) {
    ...
    this.subscriptions.push(
      this.eventBus.on<{health: number, maxHealth: number}>('player.health.changed', this.updateHealthBar.bind(this)),
      this.eventBus.on<{items: Array<{id: string, count: number}>}>('player.inventory.changed', this.updateInventory.bind(this)),
